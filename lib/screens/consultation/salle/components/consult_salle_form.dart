...
import 'package:gestionsalle/screens/consultation/salle/components/allsalle.dart';
...
class ConsultSalleForm extends StatefulWidget {
  @override
  _ConsultSalleFormState createState() => _ConsultSalleFormState();
}

class _ConsultSalleFormState extends State<ConsultSalleForm> {
  final _formKey = GlobalKey<FormState>();
  ApiService allSalle = new ApiService();
  var response;
      ...
      child: Column(
        children: [
           ListCombo<String>(
            ...
            getList: () async {
              //allSalle.getSalle()
              var data = await allSalle.getSalle();
          ...
          DefaultButton(
            ...
            press: () async {
              if (_formKey.currentState.validate()) {
                ...
                response = await getSalle.getReservationSalle(_item, _debut ,_fin);
              
                print(response);
                if (response == 401) {
                   Toast.show("Salle or Date no send", context);
                   return;
                }
                if (response.length == 0) {
                  ...
                  builder: (context) => AllSalleScreen(value : response),
                  ));
                }
              }
            },
