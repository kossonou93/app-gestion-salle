...
class VideoProjecteurForm extends StatefulWidget {
  @override
  _VideoProjecteurFormState createState() => _VideoProjecteurFormState();
}

class _VideoProjecteurFormState extends State<VideoProjecteurForm> {
  final _formKey = GlobalKey<FormState>();
 
  ApiService addVideoProjecteur = new ApiService();
  String email;
  String _item;
  String _debut;
  String _fin;
      ...
      child: Column(
        children: [
          ...
          ListCombo<String>(
            ...
            getList: () async {
              //allSalle.getSalle()
              var data = await addVideoProjecteur.getVideoPro();
            ...
            press: () async {
              if (_formKey.currentState.validate()) {
                ...
                if (response.length == 0) {
                  Toast.show("Video projecteur reservé avec succèss", context, duration: Toast.LENGTH_LONG , gravity:  Toast.CENTER);
                  Navigator.pushNamed(context, ReservationScreen.routeName);
                }else{
                  Toast.show("Vidéo projecteur est déjà reservé! ", context, duration: Toast.LENGTH_LONG , gravity:  Toast.CENTER);
                   return;
                }
